...
export default function DashboardPage() {
  ...
  // ✅ NEW: Adjustable min-trades threshold
    ...
    <main className="min-h-screen bg-gray-50 p-8 text-gray-900">
      ...
      <div className="bg-white border rounded p-4 mb-6">
        <label className="block text-sm font-medium mb-2">
          Minimum trades to evaluate a cell: <strong>{minTrades}</strong>
        </label>
        <input
          type="range"
          min={1}
        ...
        <p className="text-xs text-gray-600 mt-2">
          Cells with fewer trades are muted and won’t be flagged.
        </p>
      </div>
                    ...
                    return (
                      ...
                      >
                        {avgR === null ? (
                          ...
                          <>
                            {avgR.toFixed(2)}
                            {shouldFlag && <span className="ml-1">🚩</span>}
                          </>
                        )}
                      </td>
                    );
